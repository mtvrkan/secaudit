...
That leaves what only the real tool can answer — whether Semgrep accepts the pack — checked in CI
by running `semgrep --validate` against it. The split is deliberate: the semantics are verified
everywhere, acceptance where the tool exists.

That used to read "whether Semgrep accepts the YAML **envelope**", and the narrower wording was
wrong in a way that cost a red build. Semgrep also rejects *regexes* Python compiles happily: it
unrolls a bounded repetition where `re` walks it, so `{0,6000}` came back as `regular expression
is too large` from a pattern that is valid Python, in well-formed YAML, producing identical spans
under the equivalence test above. The generator withholds such rules now and
`test_no_exported_rule_needs_unrolling` holds it to that — but the general point is the one worth
keeping: two regex engines can disagree about a construct neither calls unusual, and no amount of
testing one of them finds it.
"""
...
def test_no_exported_rule_needs_unrolling() -> None:
    """No exported pattern may carry a bounded repetition Semgrep's engine cannot compile.

    Python's `re` walks `{0,N}`; Semgrep's engine unrolls it into N copies of the group and
    rejects the result past an internal size limit — `regular expression is too large`. The two
    engines disagree about a construct that looks entirely ordinary, and the disagreement is
    invisible to every other test here: the pattern is valid Python, the exported YAML is
    well-formed, and the equivalence test compares the two regexes with Python's engine, which
    compiles both happily.

    So this failed in CI and nowhere else, on the round that added a form-CSRF rule whose
    `{0,6000}` walk over a document is deliberate — it is what lets that rule scan a form body
    without a nested quantifier, in a repository that ships a ReDoS detector and reports exactly
    that shape in other people's code. The generator withholds such rules now; this asserts it
    kept doing so, because the failure mode is a pack that only breaks on a machine with real
    Semgrep installed.
    """
    for detector_id, rule in parsed_rules().items():
        over = [n for n in re.findall(r"\{\d*,(\d+)\}", rule["regex"])
                if int(n) > pack._UNROLL_LIMIT]
        check(not over,
              f"{detector_id}: exported with a bounded repetition of {over and max(over, key=int)}"
              f", above the {pack._UNROLL_LIMIT} Semgrep's engine will unroll — it must be "
              f"withheld by scripts/gen_semgrep_pack.py, not exported")
...
def main() -> int:
    ...
    test_no_exported_rule_needs_unrolling()
