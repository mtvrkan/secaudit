...
def main() -> int:
    ...
    # A reply with no object in it raises, and the caller counts it. It used to return `{}` on
    # the reasoning that junk must not crash the scan — but the caller never crashed on it, it
    # merged nothing and then labelled the run a success. That made "the model confirmed none of
    # these findings" and "the model said something we could not read" render identically, which
    # for a tier whose entire output is verdicts are the two answers that must never look alike.
    for junk in ("no json here", "", "I cannot help with that request.",
                 "<html><body>429 Too Many Requests</body></html>"):
        try:
            b._parse_json(junk)
            fails.append(f"_parse_json accepted a reply with no JSON object: {junk[:40]!r}")
        except UnreadableReply:
            pass
        except Exception as e:                                        # noqa: BLE001
            fails.append(f"_parse_json raised {type(e).__name__} rather than UnreadableReply "
                         f"on {junk[:40]!r}")
