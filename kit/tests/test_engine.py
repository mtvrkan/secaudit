...
def check_build_output_is_read_only_when_the_manifest_publishes_it() -> list[str]:
    """`dist/` is generated in an application and shipped in a package, and only the manifest
    knows which this is. Asserted in both directions, and with a second build directory the
    manifest does NOT name — otherwise the rule reads as "un-skip everything near a
    package.json", which would put every `.next/` cache into a report."""
    import tempfile
    fails: list[str] = []
    sink = ("const cp = require('child_process');\n"
            "module.exports = (x) => cp.exec('ls ' + x);\n")

    def scan(manifest: str | None) -> tuple[set[str], list[str]]:
        with tempfile.TemporaryDirectory() as tmp:
            for sub in ("src", "dist", "build"):
                os.makedirs(os.path.join(tmp, sub))
                with open(os.path.join(tmp, sub, "index.js"), "w", encoding="utf-8") as fh:
                    fh.write(sink)
            if manifest is not None:
                with open(os.path.join(tmp, "package.json"), "w", encoding="utf-8") as fh:
                    fh.write(manifest)
            res = engine.scan(tmp, run_deps=False, use_scanners=False)
        return {f.file.split("/")[0] for f in res.findings}, res.notes

    where, notes = scan('{"name":"p","version":"1.0.0","main":"dist/index.js"}')
    if "dist" not in where:
        fails.append("scope: `dist/` named as the package's `main` was still skipped")
    if "build" in where:
        fails.append("scope: `build/` was read although the manifest never names it")
    if not any("publishes" in n for n in notes):
        fails.append("scope: `dist/` was read and the report does not say why")

    where, _ = scan('{"name":"p","version":"1.0.0","exports":{".":{"import":"./dist/p.mjs"}}}')
    if "dist" not in where:
        fails.append("scope: a nested `exports` entry pointing into `dist/` was not read")

    where, notes = scan(None)
    if where & {"dist", "build"}:
        fails.append(f"scope: build output was read with no manifest at all: {sorted(where)}")
    if any("publishes" in n for n in notes):
        fails.append("scope: the report claims it read build output when it did not")

    where, _ = scan('{"name":"p","version":"1.0.0","main":"src/index.js"}')
    if where & {"dist", "build"}:
        fails.append("scope: build output was read although the manifest publishes `src/`")
    return fails
...
def main() -> int:
    ...
    fails += check_build_output_is_read_only_when_the_manifest_publishes_it()
