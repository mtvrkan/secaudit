...
def test_suite_main(suite):
    ...
    if not hasattr(module, "main"):
        # A pytest-only suite: every one of its tests carries an `assert`, so pytest is already
        # a verdict route for all of them and there is no `main()` for this wrapper to call.
        # `check_consistency.py`'s check 33 makes the same allowance and names the file that
        # forced it; asserting one here instead would demand a `main()` whose only job is to
        # satisfy the wrapper that exists to catch missing ones.
        pytest.skip(f"{suite} is pytest-only — every test asserts, so pytest is its verdict")
