...
def test_sbom() -> None:
    ...
    # The three fields GitHub's attestation action requires before it will call a document a
    # CycloneDX BOM: `bomFormat`, `specVersion` and `serialNumber`. The first two were asserted
    # above from the day this test was written; the third was not, `serialNumber` was emitted
    # only when a caller passed one, the CLI never passed one, and the release pipeline failed
    # on `actions/attest` with "Unsupported SBOM format" after the wheel had already been built.
    # A schema-optional field that the first real consumer treats as mandatory is mandatory.
    check(doc.get("serialNumber", "").startswith("urn:uuid:"),
          "SBOM must carry a serialNumber as an RFC 4122 URN — actions/attest rejects a "
          "CycloneDX document without one")

    # Determinism: same tree, byte-identical document. A clock in here would make two SBOMs
    # of the same product diff, which defeats using the diff to spot dependency changes — and
    # the serial number is derived from the document for the same reason, so it must not move
    # between two builds of one tree either.
