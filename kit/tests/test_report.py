...
def check_attacker_authored_evidence(fails: list[str]) -> None:
    """The scanned file writes the evidence line, so the report renders hostile input.

    Three things went through verbatim before this existed, each proven here by the thing it
    enables rather than by asserting the sanitiser's own source:

    * an ANSI escape reaching the terminal, where `\\x1b[2J` clears the screen and `\\r`
      overwrites the line the report just printed;
    * a bidi override — Trojan Source, CVE-2021-42574 — which makes the evidence line render in
      an order the file does not have, in the one place a reader is trusting the display;
    * a backtick closing the inline code span, so the rest of the line becomes live markdown in
      a report that gets pasted into a pull request.
    """
    hostile = Finding(
        detector_id="TEST-HOSTILE", title="probe", severity=Severity.HIGH,
        confidence=Confidence.HIGH, cwe="CWE-1", owasp="A01",
        file="app.py", line=1,
        # Written as escapes, never as literals. A `\u202e` pasted into this file would make
        # the line render right-to-left in every editor that opens it — the repository would be
        # demonstrating Trojan Source on its own reviewers to test that it catches it. Same
        # reasoning that made check 19's AKIA-shaped test literal get assembled at runtime.
        evidence="os.system(x)  # \x1b[31m\u202ered\u200b ` + [click](https://evil.example) + `",
        fix="n/a")

    # Sanitised at construction, not at render: every tier builds its own Finding.
    for raw, name in (("\x1b", "an ANSI escape"), ("\u202e", "a bidi override"),
                      ("\u200b", "a zero-width space")):
        if raw in hostile.evidence:
            fails.append(f"Finding kept {name} in evidence verbatim")
    for marker in ("<U+001B>", "<U+202E>", "<U+200B>"):
        if marker not in hostile.evidence:
            fails.append(f"evidence does not name the character it replaced ({marker})")

    result = ScanResult(target="probe", findings=[hostile])
    md = report.to_markdown(result)
    if "\x1b" in md or "\u202e" in md:
        fails.append("to_markdown emitted a raw control character")

    # The payload's own backticks must stay INSIDE the span. A span opened with `` closes on the
    # next ``, and the single backticks the attacker supplied are then ordinary text.
    line = next((ln for ln in md.splitlines() if "TEST-HOSTILE" not in ln
                 and "[click]" in ln), "")
    if not line:
        fails.append("the hostile evidence line is missing from the markdown report")
    elif not line.split("**", 2)[-1].lstrip(": ").startswith("``"):
        fails.append(f"evidence is not in a backtick-safe code span: {line!r}")

    # And the HTML renderer must still escape it — the reason this class was looked for at all.
    html = report.to_html(result)
    if "[click](https://evil.example)" in html and "&lt;" not in html:
        fails.append("to_html stopped escaping evidence")
...
def main() -> int:
    ...
    check_attacker_authored_evidence(fails)
