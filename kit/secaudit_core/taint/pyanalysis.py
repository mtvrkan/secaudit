...
def _py_dotted(node: ast.AST) -> str:
    ...
    **A call in the middle of the chain does not end the name.** This used to return '' the
    moment the walk hit anything but a Name, which meant a sink reached through a constructor
    was invisible:

        MongoClient(os.environ["MONGO_URL"]).ops.records.find_one(query)

    The receiver test that `_py_method_sink` performs — is the thing before the method a
    database-ish name — is still answerable there, and it is the pattern every driver's own
    documentation uses. So a Call base contributes its callee's name and the chain continues;
    anything else (a subscript, a literal, a comprehension) still ends the name, because there is
    no name left to test.
    """
    parts: list[str] = []
    ...
    def check_sink(self, call: ast.Call) -> None:
        ...
        if sink.requires_kwarg:
            ...
            if kw is None or not (isinstance(kw.value, ast.Constant) and kw.value.value):
                return
        # `tainted.read_text()` — the value the attacker chose is the receiver, and the call has
        # no argument to inspect. Checked before the argument loop because for these sinks the
        # loop has nothing to iterate over.
        if sink.taint_receiver and isinstance(call.func, ast.Attribute):
            taint = self.taint_of(call.func.value)
            if taint:
                ...
                self.paths.append(TaintPath(
                    ...
                    steps=[(call.lineno, f"{sink.id} receiver")],
