...
def to_json(result: ScanResult) -> str:
    return json.dumps({
        ...
        "tools_used": result.tools_used, "files_scanned": result.files_scanned,
...
<p class="meta"><code>{_esc(result.target)}</code> ·
 {_esc(t("html.meta", backend=result.backend, files=result.files_scanned,
         tools=', '.join(result.tools_used)))}</p>
...
def _code(text: str) -> str:
    """An inline code span that attacker-authored text cannot break out of.

    `` `{evidence}` `` was the whole implementation, and a backtick in the scanned line closed
    the span: everything after it became live markdown, so a repository could put
    `[click](https://…)` in a comment and have this tool render a working link inside its own
    report — the report that then gets pasted into a pull request. `to_html` had reasoned this
    through for HTML ("an unescaped evidence line is a scanner that plants XSS in its own
    findings page") and the same sentence is true of markdown one renderer over.

    CommonMark's own answer, rather than escaping: a span delimited by *n* backticks holds any
    run shorter than *n*, and one leading and trailing space is stripped by the parser, which is
    what keeps content that itself begins or ends with a backtick intact.
    """
    text = str(text)
    longest = 0
    run = 0
...
def to_markdown(result: ScanResult, locale: str = i18n.DEFAULT) -> str:
    ...
    lines = [
        ...
        t("report.meta", backend=result.backend, files=result.files_scanned,
    ...
    for f in result.by_severity():
        lines += [
            ...
            f"- **{t('field.location')}:** {_code(f'{f.file}:{f.line}')}",
            ...
            f"- **{t('field.evidence')}:** {_code(f.evidence)}",
        ]
        ...
        if f.taint_path:
            lines.append(f"- **{t('field.reachability')}:** {_code(f.taint_path)}")
        if f.vex_status:
