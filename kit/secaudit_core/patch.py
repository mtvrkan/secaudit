...
_ENTRY_POINTS = (
    ...
    # Written as an alternation rather than `\(?[\w,\s]*\)?\s*` — the optional parentheses
    # around a possibly-empty character class put two whitespace-matching repeats next to each
    # other, which is quadratic on a long non-matching line and, by this project's own threat
    # model, the line is attacker-authored. Same matches on 7 MB of real JavaScript.
