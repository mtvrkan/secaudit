...
CWE_TO_ASVS: dict[str, str] = {
    ...
    "CWE-98": "V1",     # PHP remote/local file inclusion — injection of a code PATH,
                        # which V1 covers for the same reason it covers CWE-94: the
                        # attacker chooses what executes.
    ...
    "CWE-614": "V3",    # Cookie without Secure — the same chapter as its HttpOnly sibling:
                        # both are properties of how the session cookie is issued.
    "CWE-1275": "V3",   # Cookie with improper SameSite
    "CWE-352": "V3",    # Cross-site request forgery
    "CWE-1021": "V14",  # Improper restriction of rendered UI layers — X-Frame-Options or a
                        # frame-ancestors policy turned off. Same chapter and same reasoning as
                        # CWE-693 below: a response header is a Configuration control.
    "CWE-693": "V14",   # Protection mechanism failure — a Content-Security-Policy that
                        # re-permits what it exists to block. V14 is Configuration, which is
                        # where ASVS puts the HTTP security headers, rather than V3 Session
                        # Management: the header is a deployment control, not a session one.
...
CWE_TO_PCI: dict[str, str] = {
    ...
    "CWE-942": "6.2.4", "CWE-1004": "6.2.4", "CWE-614": "6.2.4", "CWE-1275": "6.2.4", "CWE-22": "6.2.4",
    "CWE-98": "6.2.4",
    "CWE-434": "6.2.4", "CWE-287": "6.2.4", "CWE-384": "6.2.4", "CWE-347": "6.2.4",
    "CWE-639": "6.2.4", "CWE-284": "6.2.4", "CWE-732": "6.2.4", "CWE-862": "6.2.4",
    "CWE-841": "6.2.4", "CWE-602": "6.2.4", "CWE-306": "6.2.4", "CWE-693": "6.2.4",
    "CWE-1021": "6.2.4",
...
# The reporting clock, from Article 14. Here rather than in the page that renders it, for the
# same reason every other decaying fact in this repository is: three deadlines typed into a
# marketing page are three deadlines nobody re-reads, and getting one wrong about a regulation
# is worse than saying nothing. The trigger is what matters and is stated with them — the clock
# starts on an **actively exploited** vulnerability, not on any vulnerability, which is why
# `exploitation.py` exists and why `--watch` asks the feeds rather than the code.
CRA_REPORTING_ARTICLE = "Article 14"
CRA_DEADLINES: tuple[tuple[str, str], ...] = (
    ("24 hours", "early warning to ENISA and the CSIRT designated as coordinator"),
    ("72 hours", "vulnerability notification, with corrective or mitigating measures taken"),
