...
def build(root: str, target_name: str = "", serial_number: str = "",
    ...
    document["serialNumber"] = serial_number or _derived_serial(document)
    if timestamp:
        document["metadata"]["timestamp"] = timestamp
    return document


def _derived_serial(document: dict) -> str:
    """A `serialNumber` derived from the document, because a BOM without one is not a BOM.

    It was omitted unless a caller passed one, and the CLI never did — so `--format cyclonedx`
    emitted a document that **GitHub's attestation action refuses**: `checkIsCycloneDX` requires
    `bomFormat`, `specVersion` *and* `serialNumber`, and the release pipeline failed on the third.
    The field is optional in the CycloneDX schema and required in practice by the first consumer
    that read one of ours, which is the more useful definition of required.

    Derived rather than random, so the module's promise survives: the same tree still produces a
    byte-identical document, and a diff between two SBOMs still shows dependency changes rather
    than a fresh UUID. `uuid5` over the document's own JSON is a real RFC 4122 URN — CycloneDX
    asks for uniqueness per BOM, and two runs over identical inputs describe the same BOM.
    """
    import uuid                                                          # noqa: PLC0415

    body = json.dumps(document, sort_keys=True, separators=(",", ":"))
    return f"urn:uuid:{uuid.uuid5(uuid.NAMESPACE_URL, 'secaudit:sbom:' + body)}"
