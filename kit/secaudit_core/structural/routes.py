...
_NON_PRODUCTION = ("/tests/", "/test/", "test_", "_test.py", "tests.py", "test.py",
                   ...
                   # The other place a codebase deliberately writes fake credentials and fake
                   # data: the seeder. Every framework has one and they are named for it —
                   # Django puts them in `management/commands/seed*.py`, Rails in `db/seeds.rb`,
                   # Laravel in `database/seeders/`, and half of everyone else in a file with
                   # `seed` or `factory` in the name. Measured before it was added: across 62
                   # external repositories these paths held **36 false positives and not one
                   # true positive**, and the false ones were all the same line — a
                   # `DEMO_PASSWORD` for a local database. Same reasoning as the test entries
                   # above, which is why it belongs in the same list rather than in a rule.
                   "/management/commands/", "/seeders/", "/seeds/", "/factories/",
                   "seed.py", "seeds.py", "seed_", "_seed.py", "seeds.rb", "_factory.py")
...
class Route:
    ...
    __slots__ = ("decorators", "func", "line", "methods", "path", "path_known")

    def __init__(self, func: AnyFunc, path: str, methods: set[str],
                 decorators: list[str], path_known: bool = True) -> None:
        self.func = func
        self.path = path
        self.methods = methods
        self.decorators = decorators
        self.line = func.lineno
        # Whether `path` is what the route is mounted at, or just the empty string standing in
        # for "this framework does not put the path here". Django keeps it in `urls.py`, so
        # every Django function view and every class-based view arrives with an empty path —
        # and `public_by_design` read that empty string as the site root and answered True.
        # Every Django view in the corpus was therefore filed as deliberately public, which
        # silenced the missing-authentication rule on all of them. An unknown path is not a
        # claim about who may call it.
    ...
    def public_by_design(self) -> bool:
        ...
        if not self.path_known:
            # Judge it on the handler's name alone. The path is the stronger signal and it is
            # simply absent here; inventing "root, therefore public" from its absence is how
            # `client_edit` and `employee_create` came to be treated as deliberately open.
            return any(m in name for m in _PUBLIC_PATH_MARKERS if m != "/")
        if path in ("", "/"):
            return True
        return any(m in path or m in name for m in _PUBLIC_PATH_MARKERS if m != "/")


# Modules that serve HTTP. Several rules here are about what a *response* does, and outside a
# web module they read the wrong thing entirely: a CLI helper returning `f"failed: {e}"` to its
# own caller is not disclosing an exception to the internet, and an `int` parameter of an
# internal function is not a number the caller sent. Dogfooding this engine — which is a CLI —
# is what found both, in the commit that introduced the rules.
_WEB_MARKERS = ("django", "flask", "fastapi", "starlette", "tornado", "aiohttp", "sanic",
                "falcon", "pyramid", "bottle", "quart", "graphene", "strawberry", "rest_framework",
                "httpresponse", "jsonresponse", "jsonify", "@app.route", "@router.", "@bp.",
                "apiview", "viewset", "wsgi", "asgi")


def is_web_module(tree: ast.AST, text: str) -> bool:
    """Whether this file serves HTTP at all."""
    lowered = text.lower()
    if any(marker in lowered for marker in _WEB_MARKERS):
        return True
    return any(_route_of(node) is not None for node in ast.walk(tree)
               if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)))
...
def _route_of(func: AnyFunc) -> Route | None:
    ...
    if args and args[0] in ("request",) and not _is_method(func):
        return Route(func, "", _django_methods(func), decorators, path_known=False)
    if len(args) > 1 and args[0] == "self" and func.name.lower() in _VERB_DECORATORS:
        return Route(func, "", {func.name.lower()}, decorators,
                     path_known=False)          # class-based view
    return None
...
def _bound_values(node: ast.AST) -> list[ast.AST]:
    """The expressions a binding can actually hold, with syntax unwrapped.

    `await request.json()` is a request read wearing an `Await`, and
    `json.loads(request.body) if request.body else {}` is one wearing an `IfExp`. Both are how
    every async framework in the corpus spells the ordinary thing, and a rule that pattern-matches
    the assigned value sees neither — which cost the mass-assignment rule 21 of its 31 misses,
    all of them the same two keywords. Unwrapping is syntax, not analysis: the value of an await
    is the awaited value, and the value of a conditional is one of its two branches.
    """
    out: list[ast.AST] = []
    stack = [node]
    while stack:
