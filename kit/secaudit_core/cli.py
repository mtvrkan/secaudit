...
from . import (__version__, detectors, diff, engine, exploitation, gitref, i18n, monitor, patch,
               report, sbom, spdx)
...
def main(argv: list[str] | None = None) -> int:
    ...
    # The first thing anybody types after `pip install`, and until now it printed a usage error.
    # It matters more here than in most tools: every published figure in this project is tied to
    # the code that produced it, and a user comparing their output to a number on the site has no
    # other way to say which version they are holding. Kept in sync with `pyproject.toml` by
    # check 38 — the release workflow only ever checked the tag against the manifest.
    ap.add_argument("--version", action="version", version=f"secaudit {__version__}")
    # More than one, because pre-commit passes the staged files as separate arguments and this
    # repository ships two pre-commit hooks with `pass_filenames: true`. With a single positional
    # every commit touching two files invoked them as `secaudit ... a.py b.py`, which argparse
    # rejected — so the shipped hook failed on a usage error, not on a finding, for every commit
    # but a one-file one. Analysis still runs over the whole set at once rather than once per
    # file, so taint that crosses two staged files is still followed.
    ap.add_argument("target", nargs="+", help="file(s) or directory to audit")
    ...
    # One target is the overwhelmingly common shape, and every message and single-tree flag
    # below is written for it. `first` is that target; `args.target` stays the full set the
    # scan walks.
    first = args.target[0]
    if len(args.target) == 1 and _looks_like_a_url(first):
        print(
            f"`{first}` is a URL, and this command audits source code on disk.\n"
            f"\n"
    ...
    missing = [p for p in args.target if not os.path.exists(p)]
    if missing:
        print(
            f"`{', '.join(missing)}` does not exist, so nothing was scanned.\n"
            f"\n"
            f"Said plainly because the alternative is worse: a path that is not there walks to "
            f"zero files, and the run would finish with every count at zero, the line that says "
            f"no findings, and exit 0 — a gate that passes forever while auditing nothing. "
            f"Check the path, or the working directory this ran from "
            f"(`{os.getcwd()}`).",
            file=sys.stderr)
        return 2
    # `--patch-tests` is the command that decides whether a suggested patch is kept, so on its
    # own there is nothing for it to run against. Rejected rather than ignored: a run that
    # quietly drops the verification step looks exactly like one that verified and passed.
    if args.patch_tests and not args.suggest_patches:
        print("--patch-tests verifies suggested patches and needs --suggest-patches DIR to have "
              "produced some; on its own there is nothing for it to run.", file=sys.stderr)
        return 2
    # Checked here rather than where the patches are written, because there are two places that
    # write them — the plain path and the `--since` path — and the refusal has to be the same
    # from both. It used to print from inside the writer and return normally, so a run that
    # produced no patch at all still exited 0 and the empty output directory went to whatever
    # step came next in the script.
    if args.suggest_patches and args.backend == "none":
        print("--suggest-patches needs a model to write the patch: add "
              "--backend anthropic|openai|ollama. (What *verifies* the patch is the "
              "deterministic engine, but something still has to propose one.)", file=sys.stderr)
        return 2

    # Three flags are about a tree, not a file list: `--since` diffs one checkout against an
    # older revision of itself, `--watch` keeps state keyed to one target, `--suggest-patches`
    # copies one tree to patch it. Given several paths each would have to pick one and say
    # nothing about the rest, so they are refused instead — the CLI's own habit everywhere else
    # a combination cannot mean one thing.
    if len(args.target) > 1:
        for flag, on in (("--since", args.since), ("--watch", args.watch),
                         ("--suggest-patches", args.suggest_patches)):
            if on:
                print(f"{flag} works on one tree, and {len(args.target)} targets were given. "
                      f"Pass the directory that contains them.", file=sys.stderr)
                return 2
...
def _run_watch(args, result: ScanResult) -> int:
    ...
    if state is None:
        state = monitor.watchlist_from(result.findings, result.target, today)
        ...
        _emit(monitor.to_markdown(monitor.Report(
            target=result.target, watched_total=watched, unchanged=watched)),
...
def _suggest_patches(args, result, only) -> None:
    ...
    # `main` refuses `--suggest-patches` without a backend before either call site reaches here.
    ...
    outcomes = patch.suggest(result, result.target, backend, rescan,
...
def _run_diff(args, current, only: set[str] | None) -> int:
    ...
    try:
        # `--since` is refused above unless there is exactly one target, so this is that one.
        tree = gitref.baseline_tree(args.since, args.target[0])
