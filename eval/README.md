...
| [`cvefixes/`](cvefixes) | The third external corpus, and the first adopted **under the seal from the first minute** — a fifth of it is held out before it was ever scored, so a blind figure exists again. Labels are the lines each CVE's fix deleted. |
