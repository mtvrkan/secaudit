...
authorization:
  ...
  1. Offensive/active security scanners and exploitation frameworks (nuclei, nmap,
     sqlmap, ZAP, hydra, ffuf, nikto, wpscan, msfconsole, ...) — these send crafted
     probes/attacks.
...
ACTIVE_TOOLS = re.compile(
    ...
    # Exploitation frameworks. Absent until they were tested for, while `joomscan` and `tplmap`
    # — far narrower tools — were already here. `msfvenom` builds a payload rather than sending
    # one and is local by that reading, but the thing it builds has one purpose, and a gate that
    # blocks the delivery of a payload while waving through its construction is drawing a line
    # nobody asked for.
    # `beef-xss` and not a bare `beef`: this gate blocks, and a word that common would wedge an
    # unrelated command on a false match. A miss costs a probe that should have been asserted
    # for; a false block costs the operator's trust in the gate, and a distrusted gate is
    # switched off.
    r"msfconsole|msfvenom|msfdb|metasploit|beef-xss|sqlninja|"
...
def _selftest_scope_trust() -> list[str]:
    ...
    Two of the four cases need a real git, and without one they are unreachable rather than
    failing — `tracked_by_git` returns None, the guard refuses (correctly; case 4 is the test
    for exactly that), and their expectations can never be met. This used to guard only case 3,
    so a machine without git on PATH got a red build reporting two defects that were not
    defects, which is the same class of lie as a green build reporting no defect that was one.

    A skip is only acceptable when it cannot become the normal path, so it is refused in CI:
    every CI provider sets `CI`, and the workflow that runs this always has git because it
    checked the repository out with it.
    """
    ...
    if not have_git:
        if os.environ.get("CI"):
            return ["[scope] git is not on PATH and this is CI — two of the four scope.yaml "
                    "trust cases cannot run, and a gate that skips in CI is not a gate"]
        print("  note: git not on PATH — 2 of the 4 scope.yaml trust cases were NOT exercised: "
              "an untracked scope.yaml unlocking, and a tracked one being refused. The other "
              "two (a non-asserting file, and an undecidable one refusing) ran. Install git.")

    asserted = "owner: me\ni_am_authorized: true\n"
    cwd = os.getcwd()
    root = tempfile.mkdtemp(prefix="secaudit-scope-")
    try:
        # 1. An untracked scope.yaml the operator wrote is a valid assertion. Needs git: with
        #    none, `tracked_by_git` cannot establish the file is not a clone's, so the guard
        #    refuses it and this expectation is unreachable rather than wrong.
...
def selftest() -> int:
    ...
    block = [
        ...
        # Exploitation frameworks. These were allowed through until they were tested for, and
        # the list they were missing from already held far narrower tools.
        "msfconsole -q -x 'use exploit/multi/handler'",
        "msfvenom -p windows/meterpreter/reverse_tcp LHOST=10.0.0.1 -f exe",
        "/opt/metasploit-framework/msfconsole", "beef-xss", "sqlninja -m test -f cfg",
    ...
    allow = [
        ...
        # The other half of adding an exploitation framework to the list: a blocking gate that
        # fires on an ordinary word is worse than one that misses a tool, because the operator
        # switches it off. `beef-xss` is on the list and a bare `beef` deliberately is not.
        "echo 'beef stew' > menu.txt", "grep -r beef ./recipes",
        "python -m pytest tests/test_amass_parser.py",
    ]
    fails = []
    for c in block:
        if decide(_bash(c)) != 2:
