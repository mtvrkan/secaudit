...
backend (`anthropic` / `openai` / `ollama` / `none`). What the LLM-free tier actually measures
...
|---|---|
| Recall | **98%** (60/61 labelled vulnerabilities, 15 languages) |
| Precision | **100%** (upper bound — see the scoring rules) |
| F1 · F3 | **0.992** · **0.985** |
| False positives on safe-implementation traps | **0** / 61 |
...
> **vulnerable** one plants **61 code flaws** across **15 languages** (SQLi, command
...
> ([`secure-app`](tests/fixtures/secure-app)) implements the *same* features safely — 61
...
- **[Semgrep rules](rules/secaudit/README.md)** — 40 of the 79 detectors as a Semgrep pack, and
  the published list of which 39 are withheld because a regex rule cannot reproduce them.
